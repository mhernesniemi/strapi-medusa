  ...
  try {
    if (!ctx.query.fields?.includes("medusa_id")) {
      const fields = ctx.query.fields
        ? ctx.query.fields.push("medusa_id")
        : ["*"];
      ctx.query = {
        ...ctx.query,
        fields,
    ...
    if (!ctx.query.pagination) {
      ...
      const page =
        Math.floor(
          parseInt(ctx.query.pagination.start ?? "0") /
            parseInt(ctx.query.pagination.limit ?? "1")
        ) + 1;
      ctx.query = {
        ...ctx.query,
        page,
    ...
    try {
      ...
      try {
        ...
        switch (e.name) {
          ...
          default:
            strapi.log.error(
              `unable to create ${uid} ${e.message} ${dataReceived["medusa_id"]}`
            );
            break;
        }
        // throw e;
...
async function controllerUpdate(ctx, strapi, uid) {
  ...
  strapi.log.info(
    `Medusa is updating entity ${medusa_id} of type ${uid} in Strapi`,
    { data: data }
  );
  ...
  try {
    ...
    if (entityId) {
      ...
      const processedData = await attachOrCreateStrapiIdFromMedusaId(
        uid,
        strapi,
        data
      );
      ...
      strapi.log.warn(
        `Cannot update entity ${medusa_id} of type ${uid} as it doesnt exist in strapi`
      );
      return ctx.notFound(ctx);
    }
  } catch (e) {
